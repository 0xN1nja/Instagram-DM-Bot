...
class DMBOT():
    ...
    def login(self):
        try:
            ...
            self.driver.execute_script('document.querySelector("body > div.RnEpo.Yx5HN > div > div > div > div.mt3GC > button.aOOlW.HoLwm").click()')
        except:
    ...
    def find_target(self):
        try:
            ...
            time.sleep(2)
            self.driver.find_element_by_xpath('/html/body/div[2]/div/div/div[1]/div/div[2]/div/button/div').click()
            time.sleep(5)
        except:
    ...
    def send_message(self):
        try:
            ...
            self.target_name=str(self.driver.find_element_by_xpath('//*[@id="react-root"]/section/div/div[2]/div/div/div[2]/div[1]/div/div/div[2]/div/div[2]/button/div/div/div').get_attribute("innerHTML"))
        except:
            self.message_sent=False
        else:
            discord_notifications.notify("message-sent",CURRENT_TIME(),self.target_name)
            print(f"Birthday Wished! Target Name : {self.target_name}")
    ...
    def shutdown_pc(self):
        ...
        with open(r"message.log","a",encoding="utf-8")as f:
            f.write(f"{CURRENT_TIME()} : Birthday Wished : {self.message_sent}, Target Name : {self.target_name}"+"\n")
        # Don't Shutdown PC rn
        if SHUTDOWN:
            os.system("shutdown /s /t 1")
if __name__ == "__main__":
    dm_bot=DMBOT(CREDENTIALS["username","password"])
    dm_bot.login()
    dm_bot.find_target()
    # Scheduled
    while True:
